...
module tb_sample_fifo;
    ...
    // ── read_sample ───────────────────────────────────────────
    // sample_fifo delays data_valid by 1 cycle after rdempty goes low,
    // so by the time data_valid=1, q is settled and valid.
    // Sample data_out, then pulse fft_ready for one sysclk to pop.
    task automatic read_sample(output [23:0] d);
        d = data_out;
        fft_ready = 1;
        @(posedge sysclk); #1;   // FIFO pops on this posedge
        fft_ready = 0;
    endtask
